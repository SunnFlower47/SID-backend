...
class RecaptchaForgotPasswordMiddleware
{
    /**
     * Handle an incoming request.
     * Middleware untuk validasi reCAPTCHA v2 di forgot password
     */
    public function handle(Request $request, Closure $next): Response
    {
        // Hanya validasi untuk forgot password
        if (!$request->is('forgot-password') && !$request->is('auth/forgot-password')) {
            return $next($request);
        }
