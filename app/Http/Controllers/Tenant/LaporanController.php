...
class LaporanController extends Controller
{
    ...
    public function index()
    {
        ...
        $recentActivities = collect([
            ...
            [
                ...
                'count' => \App\Models\KartuKeluarga::whereDate('created_at', today())->count(),
                'icon' => 'fas fa-home',
    ...
    public function getReportData($type, $startDate, $endDate)
    {
        switch ($type) {
            ...
            case 'kk':
                return \App\Models\KartuKeluarga::whereBetween('created_at', [$startDate, $endDate])
                    ->get();
    ...
    public function penduduk(Request $request)
    {
        ...
        if ($request->filled('dusun_id')) {
            $query->whereHas('kartuKeluarga', fn($q) => $q->where('dusun_id', $request->dusun_id));
        }

        // Filter berdasarkan RT
        if ($request->filled('rt_id')) {
            $query->whereHas('kartuKeluarga', fn($q) => $q->where('rt_id', $request->rt_id));
        }
    ...
    {
        // Get unique KK data from Source of Truth
        $query = \App\Models\KartuKeluarga::withWilayah();
        ...
        // Get statistics from Source of Truth
        $totalKK = \App\Models\KartuKeluarga::count();
        $totalFiltered = $query->count();
