    ...
    <div class="bg-gradient-to-r from-green-600 to-green-700 rounded-2xl shadow-xl p-6 sm:p-8 text-white">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center space-x-4 mb-4 sm:mb-0">
                ...
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold">Edit Program Bantuan Sosial</h1>
                    <p class="text-green-100 text-sm sm:text-base">Ubah informasi bantuan sosial</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg border-0 overflow-hidden">
                <div class="px-6 py-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-gray-100">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900">Form Edit Program</h3>
                            <p class="text-sm text-gray-600">Ubah data program bantuan sosial dengan benar</p>
                        </div>
                    </div>
                </div>
                <div class="p-8">
                    ...
                    <form action="{{ route('bantuan-sosial.update', $bantuanSosial) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('nama_program') border-red-500 @enderror"
                                       id="nama_program" name="nama_program" value="{{ old('nama_program', $bantuanSosial->nama_program) }}"
                            ...
                            <div>
                                ...
                                <select class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('jenis_bantuan') border-red-500 @enderror"
                                    ...
                                    <option value="BLT" {{ old('jenis_bantuan', $bantuanSosial->jenis_bantuan) == 'BLT' ? 'selected' : '' }}>BLT (Bantuan Langsung Tunai)</option>
                                    <option value="PKH" {{ old('jenis_bantuan', $bantuanSosial->jenis_bantuan) == 'PKH' ? 'selected' : '' }}>PKH (Program Keluarga Harapan)</option>
                                    <option value="BPNT" {{ old('jenis_bantuan', $bantuanSosial->jenis_bantuan) == 'BPNT' ? 'selected' : '' }}>BPNT (Bantuan Pangan Non Tunai)</option>
                                    <option value="Bansos Lainnya" {{ old('jenis_bantuan', $bantuanSosial->jenis_bantuan) == 'Bansos Lainnya' ? 'selected' : '' }}>Bansos Lainnya</option>
                                </select>
                                @error('jenis_bantuan')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="mt-6">
                            ...
                            <textarea class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('deskripsi') border-red-500 @enderror"
                                      id="deskripsi" name="deskripsi" rows="4" placeholder="Deskripsikan program bantuan sosial..." required>{{ old('deskripsi', $bantuanSosial->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div>
                                ...
                                <input type="number" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('nilai_bantuan') border-red-500 @enderror"
                                       id="nilai_bantuan" name="nilai_bantuan" value="{{ old('nilai_bantuan', $bantuanSosial->nilai_bantuan) }}"
                            ...
                            <div>
                                ...
                                <input type="text" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('periode') border-red-500 @enderror"
                                       id="periode" name="periode" value="{{ old('periode', $bantuanSosial->periode) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div>
                                ...
                                <input type="date" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('tanggal_mulai') border-red-500 @enderror"
                                       id="tanggal_mulai" name="tanggal_mulai" value="{{ old('tanggal_mulai', $bantuanSosial->tanggal_mulai->format('Y-m-d')) }}" required>
                                @error('tanggal_mulai')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('tanggal_selesai') border-red-500 @enderror"
                                       id="tanggal_selesai" name="tanggal_selesai" value="{{ old('tanggal_selesai', $bantuanSosial->tanggal_selesai->format('Y-m-d')) }}" required>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div>
                                ...
                                <select class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('status') border-red-500 @enderror"
                                    ...
                                    <option value="ditangguhkan" {{ old('status', $bantuanSosial->status) == 'ditangguhkan' ? 'selected' : '' }}>Ditangguhkan</option>
                                </select>
                                @error('status')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('kuota_penerima') border-red-500 @enderror"
                                       id="kuota_penerima" name="kuota_penerima" value="{{ old('kuota_penerima', $bantuanSosial->kuota_penerima) }}"
                                       placeholder="0">
                        ...
                        <div class="mt-6">
                            ...
                            <select class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('sumber_dana') border-red-500 @enderror"
                                ...
                                <option value="APBN" {{ old('sumber_dana', $bantuanSosial->sumber_dana) == 'APBN' ? 'selected' : '' }}>APBN</option>
                                <option value="APBD" {{ old('sumber_dana', $bantuanSosial->sumber_dana) == 'APBD' ? 'selected' : '' }}>APBD</option>
                                <option value="Swasta" {{ old('sumber_dana', $bantuanSosial->sumber_dana) == 'Swasta' ? 'selected' : '' }}>Swasta</option>
                                <option value="Lainnya" {{ old('sumber_dana', $bantuanSosial->sumber_dana) == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                            </select>
                            @error('sumber_dana')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mt-6">
                            ...
                            <textarea class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 @error('kriteria_penerima') border-red-500 @enderror"
                                      id="kriteria_penerima" name="kriteria_penerima" rows="3" placeholder="Deskripsikan kriteria penerima bantuan..." required>{{ old('kriteria_penerima', is_array($bantuanSosial->kriteria_penerima) ? implode(', ', $bantuanSosial->kriteria_penerima) : $bantuanSosial->kriteria_penerima) }}</textarea>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl shadow-lg">
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        <div>
                            <h4 class="font-semibold text-gray-900">Status Saat Ini</h4>
                            <div class="mt-2">
                        ...
                        <div>
                            <h4 class="font-semibold text-gray-900">Total Penerima</h4>
                            <p class="text-2xl font-bold text-green-600">{{ $bantuanSosial->penerima_count }}</p>
                        </div>

                        <div>
                            <h4 class="font-semibold text-gray-900">Dibuat</h4>
                            <p class="text-sm text-gray-600">{{ $bantuanSosial->created_at->format('d F Y, H:i') }}</p>
                        </div>

                        <div>
                            <h4 class="font-semibold text-gray-900">Terakhir Diupdate</h4>
                            <p class="text-sm text-gray-600">{{ $bantuanSosial->updated_at->format('d F Y, H:i') }}</p>
                        </div>
                    </div>
                </div>
            </div>
