...
@section('subtitle', 'Ubah role dan permission')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center space-y-4 lg:space-y-0">
        <div>
            ...
            <p class="text-gray-600 mt-1">Ubah role dan permission yang sesuai</p>
        </div>
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('settings.roles.show', $role) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-3 rounded-lg flex items-center transition-colors shadow-md">
                <i class="fas fa-eye mr-2"></i>
                Lihat Detail
            </a>
            <a href="{{ route('settings.roles.index') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg flex items-center transition-colors shadow-md">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route('settings.roles.update', $role) }}" class="space-y-6">
                            ...
                            @foreach($permissions as $permission)
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="{{ $permissionModel->id }}"
                                           {{ in_array($permissionModel->id, old('permissions', $rolePermissionIds)) ? 'checked' : '' }}
            ...
            <div class="flex flex-col sm:flex-row gap-3 pt-6 border-t border-gray-200">
                ...
                <a href="{{ route('settings.roles.show', $role) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-3 rounded-lg flex items-center justify-center transition-colors shadow-md">
