...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-purple-600 via-purple-700 to-purple-800 rounded-2xl shadow-2xl border-0 p-6 sm:p-8">
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-white">Tambah Data Transparansi Desa</h1>
                <p class="text-purple-100 mt-1">Tambah informasi transparansi desa baru</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border-0 overflow-hidden">
        ...
        <div class="px-6 py-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-gray-100">
            ...
            <p class="text-sm text-gray-600 mt-1">Lengkapi form di bawah ini untuk menambah data transparansi desa</p>
        </div>

        <!-- Form Content -->
        <form action="{{ route('transparansi-desa.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <select id="jenis_data" name="jenis_data" required
                    ...
                    <option value="apbdes" {{ old('jenis_data') == 'apbdes' ? 'selected' : '' }}>APBDes</option>
                    <option value="proyek" {{ old('jenis_data') == 'proyek' ? 'selected' : '' }}>Proyek Desa</option>
                    <option value="laporan_keuangan" {{ old('jenis_data') == 'laporan_keuangan' ? 'selected' : '' }}>Laporan Keuangan</option>
                    <option value="dokumen_anggaran" {{ old('jenis_data') == 'dokumen_anggaran' ? 'selected' : '' }}>Dokumen Anggaran</option>
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4"
                          ...
                          placeholder="Masukkan deskripsi data transparansi">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
