...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-green-600 via-green-700 to-green-800 rounded-2xl shadow-2xl border-0 p-6 sm:p-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center mb-4 sm:mb-0">
                ...
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-white mb-1">Edit Kontak Desa</h1>
                    <p class="text-green-100 text-sm sm:text-base">Ubah data kontak dan informasi komunikasi desa</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        <form action="{{ route('kontak-desa.update', $kontakDesa) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text"
                               id="nama"
                               name="nama"
                               value="{{ old('nama', $kontakDesa->nama) }}"
                    ...
                    <div>
                        ...
                        <input type="text"
                               id="jabatan"
                               name="jabatan"
                               value="{{ old('jabatan', $kontakDesa->jabatan) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select id="jenis"
                            ...
                            <option value="kantor_desa" {{ old('jenis', $kontakDesa->jenis) == 'kantor_desa' ? 'selected' : '' }}>Kantor Desa</option>
                            <option value="kepala_desa" {{ old('jenis', $kontakDesa->jenis) == 'kepala_desa' ? 'selected' : '' }}>Kepala Desa</option>
                            <option value="sekretaris" {{ old('jenis', $kontakDesa->jenis) == 'sekretaris' ? 'selected' : '' }}>Sekretaris Desa</option>
                            <option value="bendahara" {{ old('jenis', $kontakDesa->jenis) == 'bendahara' ? 'selected' : '' }}>Bendahara Desa</option>
                            <option value="kasi_pemerintahan" {{ old('jenis', $kontakDesa->jenis) == 'kasi_pemerintahan' ? 'selected' : '' }}>Kasi Pemerintahan</option>
                            <option value="kasi_kesejahteraan" {{ old('jenis', $kontakDesa->jenis) == 'kasi_kesejahteraan' ? 'selected' : '' }}>Kasi Kesejahteraan</option>
                            <option value="kasi_pelayanan" {{ old('jenis', $kontakDesa->jenis) == 'kasi_pelayanan' ? 'selected' : '' }}>Kasi Pelayanan</option>
                            <option value="kepala_dusun" {{ old('jenis', $kontakDesa->jenis) == 'kepala_dusun' ? 'selected' : '' }}>Kepala Dusun</option>
                            <option value="ketua_rw" {{ old('jenis', $kontakDesa->jenis) == 'ketua_rw' ? 'selected' : '' }}>Ketua RW</option>
                            <option value="ketua_rt" {{ old('jenis', $kontakDesa->jenis) == 'ketua_rt' ? 'selected' : '' }}>Ketua RT</option>
                            <option value="ketua_bumdes" {{ old('jenis', $kontakDesa->jenis) == 'ketua_bumdes' ? 'selected' : '' }}>Ketua BUMDes</option>
                            <option value="puskesmas" {{ old('jenis', $kontakDesa->jenis) == 'puskesmas' ? 'selected' : '' }}>Puskesmas</option>
                            <option value="posyandu" {{ old('jenis', $kontakDesa->jenis) == 'posyandu' ? 'selected' : '' }}>Posyandu</option>
                            <option value="sekolah" {{ old('jenis', $kontakDesa->jenis) == 'sekolah' ? 'selected' : '' }}>Sekolah</option>
                            <option value="masjid" {{ old('jenis', $kontakDesa->jenis) == 'masjid' ? 'selected' : '' }}>Masjid</option>
                            <option value="lainnya" {{ old('jenis', $kontakDesa->jenis) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        @error('jenis')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select id="status_aktif"
                            ...
                            <option value="1" {{ old('status_aktif', $kontakDesa->status_aktif) == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('status_aktif', $kontakDesa->status_aktif) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                        @error('status_aktif')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea id="alamat"
                              ...
                              placeholder="Masukkan alamat lengkap">{{ old('alamat', $kontakDesa->alamat) }}</textarea>
                    @error('alamat')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select id="rw_id" name="rw_id" onchange="populateRtByRw()" required
                            ...
                            @foreach($masterRwOptions as $rw)
                                <option value="{{ $rw['id'] }}" {{ old('rw_id', $kontakDesa->rw_id) == $rw['id'] ? 'selected' : '' }}>RW {{ $rw['kode'] }} - {{ $rw['nama'] }}</option>
                            @endforeach
                        </select>
                        @error('rw_id')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select id="rt_id" name="rt_id" onchange="syncDusunFromRt()" required
                                class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent @if(!$kontakDesa->rt_id) border-red-500 bg-red-50 @else border-gray-300 @endif @error('rt_id') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <input type="text" id="dusun_display" value="{{ $kontakDesa->dusun_label }}" disabled
                               class="w-full px-4 py-3 border border-gray-100 bg-gray-50 rounded-lg text-gray-500"
                               placeholder="Otomatis dari RT">
                        <input type="hidden" name="dusun_id" id="dusun_id" value="{{ old('dusun_id', $kontakDesa->dusun_id) }}">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text"
                               id="no_telepon"
                               name="no_telepon"
                               value="{{ old('no_telepon', $kontakDesa->no_telepon) }}"
                    ...
                    <div>
                        ...
                        <input type="text"
                               id="no_hp"
                               name="no_hp"
                               value="{{ old('no_hp', $kontakDesa->no_hp) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="email"
                               id="email"
                               name="email"
                               value="{{ old('email', $kontakDesa->email) }}"
                    ...
                    <div>
                        ...
                        <input type="url"
                               id="website"
                               name="website"
                               value="{{ old('website', $kontakDesa->website) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="url"
                               id="facebook"
                               name="facebook"
                               value="{{ old('facebook', $kontakDesa->facebook) }}"
                    ...
                    <div>
                        ...
                        <input type="url"
                               id="instagram"
                               name="instagram"
                               value="{{ old('instagram', $kontakDesa->instagram) }}"
                    ...
                    <div>
                        ...
                        <input type="url"
                               id="youtube"
                               name="youtube"
                               value="{{ old('youtube', $kontakDesa->youtube) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text"
                               id="whatsapp"
                               name="whatsapp"
                               value="{{ old('whatsapp', $kontakDesa->whatsapp) }}"
                    ...
                    <div>
                        ...
                        <input type="text"
                               ...
                               value="{{ old('jam_operasional', $kontakDesa->jam_operasional) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <textarea id="deskripsi"
                                  ...
                                  placeholder="Masukkan deskripsi">{{ old('deskripsi', $kontakDesa->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @if($kontakDesa->foto)
                        <div class="mt-2">
                            <img src="{{ Storage::url($kontakDesa->foto) }}" alt="Foto saat ini" class="w-20 h-20 object-cover rounded-lg">
                ...
                <div>
                    ...
                    <input type="number"
                           id="urutan"
                           name="urutan"
                           value="{{ old('urutan', $kontakDesa->urutan) }}"
                           min="1"
...
const currentRtId = "{{ $kontakDesa->rt_id }}";
