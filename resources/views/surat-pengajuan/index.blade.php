    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        @php
            $stats = [
                ...
                ['label' => 'Diproses', 'count' => $pengajuans->where('status', 'diproses')->count(), 'color' => 'green', 'icon' => 'fas fa-spinner'],
                ['label' => 'Selesai', 'count' => $pengajuans->where('status', 'selesai')->count(), 'color' => 'purple', 'icon' => 'fas fa-check-double'],
            ];
        @endphp
            ...
            <div>
                ...
                <select name="jenis_surat" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">
                    ...
                    @foreach($suratTypes as $type)
                        <option value="{{ $type->id }}" {{ request('jenis_surat') == $type->id ? 'selected' : '' }}>
                            {{ $type->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">
                                {{ $pengajuan->penduduk->nama ?? 'Penduduk Tidak Ditemukan' }}
                            </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg border-0 p-4 sm:p-6 hover:shadow-xl transition-all duration-300 w-full">
                ...
                <div class="space-y-3 mb-4">
                    ...
                    <div class="bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl p-3">
                        ...
                        <p class="text-sm text-gray-900 font-medium">{{ $pengajuan->penduduk->nama ?? 'Penduduk Tidak Ditemukan' }}</p>
