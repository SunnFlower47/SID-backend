...
<head>
    <meta charset="UTF-8">
    <title>Buku Peraturan di Desa</title>
    <style>
        ...
        .clear { clear: both; }
        .badge-status { display: inline-block; padding: 1px 6px; border-radius: 3px; font-size: 8px; font-weight: bold; }
        .status-disetujui { background: #d1fae5; color: #065f46; border: 1px solid #6ee7b7; }
...
<body>
    ...
    <div class="header">
        <h3>Buku Peraturan di Desa</h3>
        <p class="subtitle">(Lampiran IV — Permendagri No. 47 Tahun 2016)</p>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width:14%">JENIS PERATURAN</th>
                <th style="width:14%">NOMOR PERATURAN</th>
                <th style="width:10%">TANGGAL DITETAPKAN</th>
                <th style="width:10%">TAHUN ANGGARAN</th>
                <th>JUDUL / TENTANG</th>
                <th style="width:8%">STATUS</th>
                <th style="width:14%">KETERANGAN / CATATAN BPD</th>
            </tr>
            <tr class="col-number">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="center">{{ $row->jenis_peraturan ?? '-' }}</td>
                <td class="center">{{ $row->nomor_peraturan ?? '-' }}</td>
                <td class="center">
                    {{ $row->tanggal_ditetapkan ? \Carbon\Carbon::parse($row->tanggal_ditetapkan)->translatedFormat('d F Y') : '-' }}
                </td>
                <td class="center">{{ $row->tahun_anggaran ?? '-' }}</td>
                <td class="left">{{ $row->judul ?? '-' }}</td>
                <td class="center">
                    <span class="badge-status status-{{ $row->status ?? 'draft' }}">
                        {{ strtoupper($row->status ?? 'Draft') }}
                    </span>
                </td>
                <td class="left">{{ $row->keterangan_bpd ?? '-' }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="8" class="center" style="padding:14px; font-style:italic; color:#666;">
                    Nihil — Tidak ada data peraturan desa.
                </td>
            </tr>
            @endforelse
        </tbody>
