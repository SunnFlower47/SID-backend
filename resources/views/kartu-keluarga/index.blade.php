...
function deleteKK(nkk) {
    Swal.fire({
            ...
            .then(response => {
                ...
                return response.text().then(text => {
                    try {
                        ...
                        // If not JSON, it's a redirect response
                        location.reload();
                        return null;
                    }
                });
        ...
        .catch(error => {
            ...
            Swal.fire({
                title: 'Error!',
                text: 'Terjadi kesalahan saat menghapus Kartu Keluarga',
                icon: 'error',
                confirmButtonText: 'OK'
            });
        });
...
function batchUpdateKepalaKeluarga() {
    Swal.fire({
        title: 'Konfirmasi Batch Update',
        text: 'Apakah Anda yakin ingin memperbaiki semua KK bermasalah sekaligus? Sistem akan otomatis mencari kepala keluarga baru untuk setiap KK yang bermasalah.',
        icon: 'question',
        showCancelButton: true,
        ...
        fetch('/kartu-keluarga/batch-update-kepala-keluarga', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                'Content-Type': 'application/json',
        ...
        .then(data => {
            if (data.success) {
                Swal.fire({
                    title: 'Berhasil!',
                    html: `Batch update selesai!<br><br>Total diproses: ${data.data.total_processed}<br>Berhasil: ${data.data.success_count}<br>Gagal: ${data.data.error_count}`,
                    icon: 'success',
                    confirmButtonText: 'OK'
                });
                location.reload();
                ...
                Swal.fire({
                    title: 'Error!',
                    text: 'Gagal melakukan batch update: ' + data.message,
                    icon: 'error',
                    confirmButtonText: 'OK'
                });
            }
        ...
        .catch(error => {
            ...
            Swal.fire({
                title: 'Error!',
                text: 'Terjadi kesalahan saat melakukan batch update',
                icon: 'error',
                confirmButtonText: 'OK'
            });
        })
