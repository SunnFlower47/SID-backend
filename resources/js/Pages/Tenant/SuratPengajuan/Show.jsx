...
import { 
    ...
    Download, Settings2, PenTool, Type
} from 'lucide-react';
...
export default function Show({ auth, suratPengajuan, statusList, suratType, bsreConfigured = false }) {
    ...
    const getStatusStyle = (status) => {
        switch (status) {
            case 'selesai':
                return { bg: 'bg-green-100', border: 'border-green-200', text: 'text-green-700', icon: <BadgeCheck className="w-5 h-5 mr-1.5" /> };
            case 'diproses':
                return { bg: 'bg-blue-100', border: 'border-blue-200', text: 'text-blue-700', icon: <Clock className="w-5 h-5 mr-1.5" /> };
            case 'ditolak':
                return { bg: 'bg-red-100', border: 'border-red-200', text: 'text-red-700', icon: <XCircle className="w-5 h-5 mr-1.5" /> };
            default:
                return { bg: 'bg-yellow-100', border: 'border-yellow-200', text: 'text-yellow-700', icon: <Clock className="w-5 h-5 mr-1.5" /> };
        }
    };
        ...
        <AuthenticatedLayout user={auth.user} title="Detail Pengajuan Surat">
            ...
            <div className="space-y-6 animate-in fade-in duration-700 pb-20">
                ...
                <PageHeader 
                    ...
                    subtitle={`ID: #${p.id} • Dibuat pada ${dayjs(p.created_at).format('DD MMMM YYYY HH:mm')}`}
                    icon={FileSignature}
                            ...
                            <div className="p-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="md:col-span-2">
                                        <InfoRow label="Keperluan / Maksud" value={p.keperluan || '-'} icon={FileText} color="gray" />
                                    </div>
                                    {p.tujuan && (
                                        <div className="md:col-span-2">
                                            <InfoRow label="Tujuan Penggunaan" value={p.tujuan} icon={MapPin} color="gray" />
                                        </div>
                                    )}
                                </div>
                            </div>
                                ...
                                <div className="p-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                                        {Object.entries(p.data_tambahan).map(([key, value]) => {
                                            ...
                                            return (
                                                <InfoRow key={key} label={label} value={value?.toString() || '-'} icon={FileText} color="blue" />
                                            );
                                        })}
                                    </div>
                                </div>
