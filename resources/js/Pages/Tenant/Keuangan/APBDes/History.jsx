...
import { History, ArrowLeft, Plus, Edit2, Trash2, Save, X, Calendar } from 'lucide-react';
...
export default function HistoryPage({ auth, apbdes }) {
    ...
    const handleDelete = (id, nama) => {
        Swal.fire({
            ...
            html: `Hapus <b class="text-red-600">${nama}</b>?<br><small class="text-gray-400 font-bold uppercase tracking-widest text-[9px]">Realisasi anggaran akan berkurang</small>`,
            ...
            if (r.isConfirmed) router.delete(route('anggaran.delete-pengeluaran', id), { preserveScroll: true });
        });
    };
        ...
        <AuthenticatedLayout user={auth.user} title="Histori Pengeluaran">
            ...
            <div className="space-y-6 animate-in fade-in duration-700 pb-20">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {/* Add Form */}
                    {showAddForm && (
                        <div className="lg:col-span-2 bg-white rounded-2xl border border-green-100 shadow-sm p-6 space-y-5 animate-in slide-in-from-top-3 duration-300">
                            <div className="flex items-center justify-between mb-2">
                                <h3 className="text-sm font-black text-gray-900 uppercase italic tracking-tighter">Tambah Pengeluaran Baru</h3>
                            ...
                            <form onSubmit={handleAdd} className="space-y-4">
                                ...
                                <InputField label={`Jumlah (Sisa Anggaran: ${formatRupiah(sisaAnggaran)})`} error={errors.jumlah}>
                                ...
                                <div className="flex gap-3">
                                    <button type="button" onClick={() => { setShowAddForm(false); reset(); }}
                                        className="flex-1 py-3 rounded-xl bg-gray-50 text-gray-600 text-xs font-black uppercase tracking-widest hover:bg-gray-100 border border-gray-200 transition-all">
                                        BATAL
                                    </button>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    {apbdes.histori_pengeluarans?.length > 0 ? (
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-gray-50/80 border-b border-gray-100">
                                        {['Tanggal', 'Nama Pengeluaran', 'Jumlah', 'Keterangan', 'Aksi'].map(h => (
                                    ...
                                    {apbdes.histori_pengeluarans.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50/50 transition-all">
                                            ...
                                            <td className="px-4 py-4">
                                                <p className="text-xs font-black text-gray-900">{item.nama_pengeluaran}</p>
                                            </td>
                                            <td className="px-4 py-4 whitespace-nowrap">
                                                <span className="text-sm font-black text-blue-600">{formatRupiah(item.jumlah)}</span>
                                            </td>
                                            <td className="px-4 py-4 max-w-[200px]">
                                                <p className="text-xs text-gray-500 font-bold line-clamp-2">{item.keterangan ?? '-'}</p>
                                            </td>
                                            <td className="px-4 py-4 whitespace-nowrap">
                                                <div className="flex items-center gap-1">
                                                    <Link href={route('anggaran.edit-pengeluaran', item.id)}
                                                        className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all" title="Edit">
                                                        <Edit2 className="w-3.5 h-3.5" />
                                                    </Link>
                                                    <button onClick={() => handleDelete(item.id, item.nama_pengeluaran)}
                                                        className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all" title="Hapus">
                                    ...
                                    <tr className="bg-gray-50/80 border-t border-gray-100">
                                        ...
                                        <td className="px-4 py-3">
                                            <span className="text-sm font-black text-blue-700">
                                                {formatRupiah(apbdes.histori_pengeluarans.reduce((sum, i) => sum + Number(i.jumlah), 0))}
                                            </span>
                                        </td>
                                        <td colSpan={2} />
                                    </tr>
                        ...
                        <div className="p-16 text-center">
                            ...
                            <p className="text-[10px] text-gray-300 font-bold uppercase tracking-widest mt-2 mb-6">Klik tombol "Tambah Pengeluaran" untuk mencatat realisasi</p>
