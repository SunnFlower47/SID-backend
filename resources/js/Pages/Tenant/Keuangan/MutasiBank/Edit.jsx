...
export default function Edit({ auth, mutasiBank }) {
    const { data, setData, put, processing, errors } = useForm({
        tanggal_mutasi: mutasiBank.tanggal_mutasi ? new Date(mutasiBank.tanggal_mutasi).toISOString().split('T')[0] : '',
        jenis_mutasi: mutasiBank.jenis_mutasi,
        uraian: mutasiBank.uraian || '',
        jumlah: mutasiBank.jumlah || '',
        no_bukti: mutasiBank.no_bukti || '',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('keuangan.mutasi-bank.update', mutasiBank.id));
    };

    return (
        <AuthenticatedLayout user={auth.user} title="Edit Transaksi Bank">
            <div className="space-y-6 pb-20">
                <PageHeader
                    title="Edit Transaksi"
                    subtitle="Perbarui data setoran atau penarikan di rekening desa"
                    icon={Landmark}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
                            ...
                            <button 
                                type="submit" 
                                disabled={processing} 
                                className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs tracking-widest uppercase shadow-lg shadow-blue-900/20 transition-all disabled:opacity-50 flex items-center gap-2"
                            >
