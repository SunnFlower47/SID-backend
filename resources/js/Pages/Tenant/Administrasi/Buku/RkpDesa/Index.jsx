...
export default function RkpDesa({ auth, data, filters }) {
    const tableHead = (
        <>
            <tr className="bg-gray-100 text-gray-900 font-bold uppercase text-[10px] tracking-wider border-b border-gray-200 whitespace-nowrap">
                ...
                <th rowSpan="2" className="px-4 py-3 border-r border-gray-200 align-middle">SASARAN / MANFAAT</th>
                <th rowSpan="2" className="px-4 py-3 border-r border-gray-200 align-middle text-center">SUMBER DANA</th>
                <th rowSpan="2" className="px-4 py-3 border-r border-gray-200 align-middle text-right">JUMLAH<br/>ANGGARAN</th>
                <th rowSpan="2" className="px-4 py-3 border-r border-gray-200 align-middle text-center">WAKTU<br/>PELAKSANAAN</th>
                <th rowSpan="2" className="px-4 py-3 border-r border-gray-200 align-middle text-center">PELAKSANA</th>
                <th rowSpan="2" className="px-4 py-3 border-r border-gray-200 align-middle text-center">SIFAT<br/>PROYEK</th>
    ...
    const renderRow = (item, index, no) => {
        ...
        return (
            <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors whitespace-nowrap">
                ...
                <td className="px-4 py-3 border-r max-w-[200px] truncate" title={item.sasaran}>{item.sasaran || '-'}</td>
    ...
    return (
        <BukuLayout
            auth={auth}
            jenis_buku="rkp-desa"
            judul="Buku Rencana Kerja Pembangunan"
            data={data}
            filters={filters}
            tableHead={tableHead}
            renderRow={renderRow}
