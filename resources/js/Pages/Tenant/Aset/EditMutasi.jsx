...
export default function EditMutasi({ auth, mutasi, inventaris }) {
    const { data, setData, put, processing, errors } = useForm({
        tahun:      mutasi.tahun,
        semester:   mutasi.semester,
        tanggal:    mutasi.tanggal,
        jenis:      mutasi.jenis,
        kwantitas:  mutasi.kwantitas,
        nilai:      mutasi.nilai,
        keterangan: mutasi.keterangan || '',
        kondisi:    mutasi.kondisi || '',
        alasan_kurang: mutasi.alasan_kurang || '',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('aset.mutasi.update', mutasi.id));
    };
    ...
    // Saldo hipotetis = Saldo saat ini dikurangi data mutasi lama (karena ini diedit)
    let saldoHipotetis = inventaris.saldo_kwantitas;
    if (mutasi.jenis === 'tambah') saldoHipotetis -= parseFloat(mutasi.kwantitas) || 0;
    else saldoHipotetis += parseFloat(mutasi.kwantitas) || 0;

    let nilaiSaldoHipotetis = inventaris.saldo_nilai;
    if (mutasi.jenis === 'tambah') nilaiSaldoHipotetis -= parseFloat(mutasi.nilai) || 0;
    else nilaiSaldoHipotetis += parseFloat(mutasi.nilai) || 0;

    const saldoBaru = isTambah
        ? saldoHipotetis + (parseFloat(data.kwantitas) || 0)
        : saldoHipotetis - (parseFloat(data.kwantitas) || 0);
    const nilaiSaldoBaru = isTambah
        ? nilaiSaldoHipotetis + (parseFloat(data.nilai) || 0)
        : nilaiSaldoHipotetis - (parseFloat(data.nilai) || 0);
    const isOverSaldo = !isTambah && (parseFloat(data.kwantitas) || 0) > saldoHipotetis;

    return (
        <AuthenticatedLayout user={auth.user} title="Edit Mutasi Aset">
            <Head title="Edit Mutasi Aset" />
            ...
            <div className="space-y-5 animate-in fade-in duration-500 pb-20">
                ...
                <PageHeader
                    ...
                    backHref={route('aset.inventaris.edit', inventaris.id)}
                />
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-4">
                            {[
                                ...
                                <button
                                    key={val}
                                    type="button"
                                    disabled={true} // Jenis mutasi tidak disarankan diedit
                                    className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all text-left opacity-70 cursor-not-allowed ${
                                        data.jenis === val
                                            ? color === 'green'
                            ...
                            <FormField label={`Kwantitas (${inventaris.satuan})`} required error={errors.kwantitas}>
                                ...
                                {isOverSaldo && (
                                    <p className="text-[10px] font-bold text-red-600 uppercase tracking-tight ml-1 flex items-center gap-1 mt-1">
                                        ...
                                        Melebihi batas aman ({fmtQty(saldoHipotetis)} {inventaris.satuan})
                                    </p>
                                )}
                            </FormField>
                    ...
                    <div className="flex gap-3">
                        <Link href={route('aset.inventaris.edit', inventaris.id)}
                        ...
                        <button type="submit" disabled={processing || isOverSaldo}
                            ...
                            {processing ? 'Menyimpan...' : 'Simpan Perubahan Mutasi'}
                        </button>
                    </div>
                </form>
            </div>
