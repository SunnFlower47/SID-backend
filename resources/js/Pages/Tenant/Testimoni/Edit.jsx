                ...
                <div className="bg-gradient-to-r from-indigo-600 via-indigo-700 to-indigo-900 rounded-[3rem] shadow-2xl p-8 sm:p-10 relative overflow-hidden text-left">
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <h1 className="text-2xl sm:text-4xl font-black text-white tracking-tighter uppercase italic leading-none">Edit Testimoni</h1>
                                <p className="text-indigo-100 font-bold text-xs sm:text-sm uppercase tracking-[0.2em] mt-2 opacity-80 italic">Memperbarui data testimoni warga</p>
                            </div>
                        </div>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 sm:p-10">
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 block">Kategori & Status</label>
                                        <div className="flex gap-4">
                                            <select value={data.kategori} onChange={e => setData('kategori', e.target.value)} className="flex-1 px-6 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold shadow-inner">
                                                <option value="umum">Umum</option>
                                                <option value="pelayanan">Pelayanan</option>
                                                <option value="pembangunan">Pembangunan</option>
                                            </select>
                                            <select value={data.status} onChange={e => setData('status', e.target.value)} className="flex-1 px-6 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold shadow-inner">
                        ...
                        <div className="flex justify-end">
                            <button type="submit" disabled={processing} className="px-12 py-5 bg-indigo-600 text-white rounded-[2rem] text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-3">
                    ...
                    <div className="space-y-8">
                        <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            <div className="p-8 space-y-5">
                                ...
                                <input type="email" placeholder="Email" value={data.email} onChange={e => setData('email', e.target.value)} className="w-full px-6 py-3.5 bg-gray-50 border-none rounded-2xl text-sm font-bold shadow-inner" />
                                <input type="text" placeholder="Telepon" value={data.telepon} onChange={e => setData('telepon', e.target.value)} className="w-full px-6 py-3.5 bg-gray-50 border-none rounded-2xl text-sm font-bold shadow-inner" />
                            </div>
                        </div>
                        <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                            <div className="px-8 py-5 border-b border-gray-50 bg-gray-50/50 uppercase font-black text-[10px] text-gray-400 italic tracking-widest flex items-center gap-2">
